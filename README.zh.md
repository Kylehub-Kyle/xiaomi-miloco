...
- **2026-09-11** — 发布 v2026.9.11：新增规则 / 任务分层重构（一个任务可挂多条规则，进入与退出不必互为反面）、统一的进程内状态容器（由设备上下线与属性推送保持实时）、规则动作直接触发米家场景；并恢复相机 IP 直连（原生库已修复崩溃）与拉流静默自愈、用量按模型与 endpoint 分开记账、HEIC/HEIF 上传，以及 Smart Crop 逐机位开关。
